...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    #makes a temporary city
    temp_city = make_city('temp_city', lat, lon)

    #if the distance from temp_city to city 1 is shorter return name(city1)
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    #first check if the column is full, if which don't do anything and just return -1 and the board
    #to get the most bottom row it is max_rows - 1
    put_piece_pos = -1
    for row in range(max_rows -1, -1, -1): #iterating backwards from the biggest row
        if(get_piece(board, row, column) == '-'):
            put_piece_pos = row  #sets the return position to row
            board_row = replace_elem(board[row], column, player) #creates a boards row that replaces the column with the player
            board = replace_elem(board, row, board_row) #creates a board that replaces a cretain rowwith the board's row
            break

    return put_piece_pos, board
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    put_piece_pos = -1
    for row in range(max_rows - 1, -1, -1): #iterating backwards from the biggest row
        if(col < max_cols and col >= 0 and get_piece(board, row, col) == '-'):
            put_piece_pos = row  #sets the return position to row
            board_row = replace_elem(board[row], col, player) #creates a boards row that replaces the column with the player
            board = replace_elem(board, row, board_row) #creates a board that replaces a cretain rowwith the board's row
            break
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for a in range(max_cols):
        if(get_piece(board, row, a) == player):
            num_adjecent += 1
            if(num_adjecent == num_connect):
                return True
        else:
             num_adjecent = 0
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    "*** YOUR CODE HERE ***"
    num_adjecent = 0
    for a in range(max_rows):
        if(get_piece(board, a, col) == player):
            num_adjecent += 1
            if(num_adjecent == num_connect):
                return True
        else:
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    "*** YOUR CODE HERE ***"
    return check_win_diagonal(board, max_rows, max_cols, num_connect, row, col, player) or check_win_row(board, max_rows, max_cols, num_connect, row,player) or check_win_column(board, max_rows, max_cols, num_connect, col, player)
