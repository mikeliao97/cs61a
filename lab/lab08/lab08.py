...
def make_advanced_counter_maker():
    ...
    """
    "*** YOUR CODE HERE ***"
    #a inner function that takes commands
    #1. 'count' --> returns the instance's count and increments global, and instance counts
    #2 'global-count' 
    global_counter = 0
    def inner():
        personal_counter = 0
        def inner_inner(command):
...
def trade(first, second):
    ...
    trade = False
    results = []
    for first_list in range(1, len(first)):
        for second_list in range(1, len(second)):
            if(sum(second[:second_list]) == sum(first[:first_list])):
                m = first_list
                n = second_list
                results += [[m, n]]
                trade = True
