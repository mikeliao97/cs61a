...
def guess_binary():
    """Return the number of attempteed guesses. Implement a faster search
    ...
    "*** YOUR CODE HERE ***"
    while not correct:
        correct = is_correct(guess) #get the first guess
        #edge case is if guess is equal to UPPER - 1, then you gotta check for it
        if not correct:
            if(guess == UPPER - 1): #the edge case
                correct = is_correct(guess + 1)
                num_guesses = num_guesses + 1
                break
