...
def num_songs(t):
    ...
    if(is_leaf(t)):
        ...
        branches = t[1:]
        return sum([num_songs(b) for b in branches])
...
def add_song(t, song, category):
    ...
    branches = t[1:]
    if(root(t) == category):
        return tree(root(t), [add_song(b, song, category) for b in branches] + [tree(song)])
    else:
        return tree(root(t), [add_song(b, song, category) for b in branches])
