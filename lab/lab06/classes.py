...
class Player(object):
    ...
    def go_to(self, location):
        ...
        "*** YOUR CODE HERE ***"
        if destination_place.name == location:
            if not destination_place.locked:
                self.place = destination_place
            print_str = "You are at " + self.place.name
            print(print_str)
        else:
            return "Can't go to " + destination_place.name + " from " + self.name 
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            if person not in self.place.characters.keys():
                print_str = person + " is not here."
                print(print_str)
            else:
                person_obj = self.place.characters[person]
                print_str = person + " says: " + person_obj.talk()
                print(print_str)
                 
#Check if the person is at your current location
#person is a string. so you gotta find the characters.
#if he is at current location, print out what he says
#if he isnt, print out person is not here
...
#check if the thing string is in the keys of self.place.things.keys
            things = self.place.things
            if(thing in things.keys()):
               self.backpack.append(self.place.take(thing))
               print_str = self.name + " takes the " + thing
               print(print_str)
            else:
                print_str = thing + " is not here."
                print(print_str)
    ...
    def unlock(self, place):
        ...
        "*** YOUR CODE HERE ***"
        if(key == None):
            print_str = place + " can't be unlocked without a key!"
            print(print_str)
        else:
            location = self.place.get_neighbor(place)
            Key.use(key, location)
...
class Key(Thing):
    def use(self, place):
        if(place.locked == True):
            place.locked = False
            print_str = place.name + " is now unlocked!"
            print(print_str)
        else:
            print_str = place.name + " is already unlocked!"
            print(print_str)
