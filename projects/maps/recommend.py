...
def find_closest(location, centroids):
    ...
    # BEGIN Question 3
    "*** REPLACE THIS LINE ***"
    return min([x for x in centroids], key=lambda y: distance(location, y))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** REPLACE THIS LINE ***"
    #for each restaurant get the location and then find the closest centroid
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** REPLACE THIS LINE ***"
    #you have to find the mean longitude and mean latitude of each cluster
    #each cluster is an array of names ["top dog", "fdsaf", "fsda"]
    #each name is an array of coordinates [[3, -1], [3, 2], [2, 1]]
    # you want to zip the array of coordinates and find the zip
    restaurant_coordinates = [restaurant_location(x) for x in cluster]
    lattitude = [x[0] for x in restaurant_coordinates]
    longitude = [x[1] for x in restaurant_coordinates]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** REPLACE THIS LINE ***"
    return max([find_predictor(user,reviewed,x)[0] for x in feature_fns], key=lambda x:find_predictor(user, reviewed, x)[1])
    # END Question 8
