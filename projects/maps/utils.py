...
def key_of_min_value(d):
    ...
    # BEGIN Question 0
    "*** REPLACE THIS LINE ***"
    return min([x for x in d.keys()], key=lambda y: d[y])
    # END Question 0
...
def enumerate(s, start=0):
    ...
    """
    # BEGIN Question 0
    range_values = [x for x in range(start, len(s))]
    zip_values = [x for x in zip(s)]
    return [r for r in zip(range(start, start + len(s)), s)]
