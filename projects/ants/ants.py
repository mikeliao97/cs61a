    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is None:
                ...
                # BEGIN Problem 7
                "*** REPLACE THIS LINE ***"
#This means there is already an ant here
                #this means that the bodyguard is there first
                if(self.ant.container == True and insect.container == False): 
                    if(self.ant.ant):
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                    else:
                        self.ant.ant = insect
                elif(self.ant.container == False and insect.container == True):
                    non_bodyguard_ant = self.ant
                    self.ant = insect #set the place's ant to the bodyguard
                    insect.ant = non_bodyguard_ant
                else:
                    assert self.ant is None, 'Two ants in {0}'.format(self)
                # END Problem 7
        else:
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if hasattr(self.ant.ant, 'is_queen') and self.ant.ant.is_queen:
...
class ThrowerAnt(Ant):
    ...
    min_range = -10000 #sentinel
    max_range = 10000 #sentinel
    name = 'Thrower'
    implemented = True
    damage = 1
    food_cost = 4
    ...
    def nearest_bee(self, hive):
        ...
        location = self.place
        distance = 0
        while(location.name != hive.name):
            if(random_or_none(location.bees) != None):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        #If ant lets you create a queen, then set is queen to queen
        if(Ant.can_create_queen):
            Ant.__init__(self, 1) 
            self.is_queen = True
            self.buffed_minions = []
            Ant.can_create_queen = False
        else:
            Ant.__init__(self, 1)
            self.is_queen = False
        #how do i make sure only the first one is legit
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        #if this is the queen, do the actions
        if(self.is_queen):
            #the queen throws
            ThrowerAnt.throw_at(self, ThrowerAnt.nearest_bee(self, colony.hive))

            #create a list of all the ants behind queen. ants_behind_queen
            ants_behind_queen = []
            location = self.place.exit
            while(location != None):
                if(location.ant): #if there is an ant in the location
                    if(location.ant.container): #if ant is a container ant add both ants
                        ants_behind_queen.append(location.ant)
                        if(location.ant.ant):
                            ants_behind_queen.append(location.ant.ant)
                    else:
                        ants_behind_queen.append(location.ant)
                location = location.exit

            #if element in ants_behind_queen not in buffed_minions
            for ant in ants_behind_queen:
                if ant in self.buffed_minions:
                    pass
                else:
                    self.buffed_minions.append(ant)
                    ant.damage = ant.damage * 2
        else:
            self.reduce_armor(self.armor)

        #else reduce self.reduce_armor(self.armor)
        # END Problem 9
