...
#if there is a . that means therse only one more expression left.
#it simply means to return the rest of the list
            # BEGIN Question 2
            src.pop() #get rid of the period
            dotted_element = scheme_read(src)
            if(src.current() != ')'):
               raise SyntaxError('expected one element after .')
            src.pop()
            return dotted_element
            # END Question 2
        else:
