...
def apply_primitive(procedure, args_scheme_list, env):
   ...
   # Convert a Scheme list to a Python list
    ...
    # BEGIN Question 4
    "*** REPLACE THIS LINE ***"
    should_use_env = procedure.use_env
    try:
        if(should_use_env):
            args.append(env)
...
#changed the second expression from return scheme_eval(expressions.first, env)
...
def make_call_frame(procedure, args, env):
    ...
    if(isinstance(procedure, LambdaProcedure)):
        ...
        return procedure.env.make_child_frame(formals, args)
    else:
        formals = procedure.formals
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        while(parent_frame is not None):
            if(symbol in parent_frame.bindings):
    ...
    def make_child_frame(self, formals, vals):
        ...
        temp_bindings = {}
        while formals:
           temp_bindings[formals.first] = vals.first #set teh value
           formals = formals.second
           vals = vals.second
          

        child = Frame(self) # Create a new child with self as the parent
        child.bindings = temp_bindings
        # BEGIN Question 10
        "*** REPLACE THIS LINE ***"
        # END Question 10
        return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        "*** REPLACE THIS LINE ***"
        variable = target.first #this is the varaible
        #create a new lambda expression 
        check_formals(target.second)
        lambda_expression = LambdaProcedure(target.second, expressions.second, env)
        env.define(variable, lambda_expression)
        return variable
        # END Question 9A
    else:
...
#for do if form i changed the last statement to be true scheme_eval recursive
def do_if_form(expressions, env):
    """Evaluate an if form."""
    check_form(expressions, 2, 3)
    # BEGIN Question 13
    "*** REPLACE THIS LINE ***"
#Evaluate the condition. if the condition is true
#Check the length of the expressions. If the length of the expression is only 2
#if true, check the length
    condition = scheme_true(scheme_eval(expressions.first, env))
    if(condition):
...
#for do_and_form i put True for the last expression
def do_and_form(expressions, env):
    """Evaluate a short-circuited and form."""
    # BEGIN Question 14B
    "*** REPLACE THIS LINE ***"
    #if expressions is empty then return True
#return the last expressions if there is no more. meaning expressions.second is nil
#if any of the expressions is False return False
    if(len(expressions) == 0):
        return True
    else:
        while(expressions.second): #check for false conditions except for the last value, this doesn't check last value
            #first get the value
            value = expressions.first
            value = scheme_eval(expressions.first, env)

            if(scheme_false(value)): #if value equals false then retrn falsej
                return False
...
#Fix this probably buggy add some more tests
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_true(test):
            if(clause.first == "else" or len(clause) > 2):
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        target = bindings.first #get one part of bindings
...
def check_formals(formals):
    ...
    "*** REPLACE THIS LINE ***"
    checked_symbols = []
#1. Check wether its a well-formed list. AKA make sure each second is each a pair instance or nil
#2. Check wether any symbol is repeated

    while formals:
        #check if its a well formed list
        if(not (isinstance(formals.second, Pair) or formals.second == nil) ):
            raise SchemeError
        #check that each is a well formed symbol
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    if tail:
        ...
        "*** REPLACE THIS LINE ***"
        # END Extra Credit
        return Evaluate(expr, env) #if its a tail then just let while statement evaluate it
    else:
